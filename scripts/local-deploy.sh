...
BACKUP_DIR="/etc/${PROJECT_NAME}-backup-$(date +%F-%H%M%S)"
CRON_SOURCE_FILE="${REPO_DIR}/cron/pivpi-tv.cron" # Keep source name for now unless renamed in repo
CRON_TARGET_FILE="/etc/cron.d/${PROJECT_NAME}"
SCRIPTS_TARGET_DIR="/usr/local/bin/${PROJECT_NAME}"
COLLECTOR_SCHEMA_TARGET_FILE="/etc/${PROJECT_NAME}/collector-api-schema.json"

echo "=== ${PROJECT_NAME} Local Deployment Script ==="
...
echo "  • Install crontab from ${CRON_SOURCE_FILE} → ${CRON_TARGET_FILE}"
...
echo "6) Deploying scripts to ${SCRIPTS_TARGET_DIR}/"
